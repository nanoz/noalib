...
The iPhone 16 Pro layout target is `402 x 874` CSS pixels, matching Apple's `1206 x 2622` native display resolution at 3x. The layout uses the full layout viewport for positioning because iOS Home Screen apps can report a shorter visual viewport below the status area; using that reduced height would shift the mockup upward under the real time, signal, and battery indicators. The real iOS status bar remains visible over the green header. Only the duplicate status-bar artwork baked into the PNG is masked, so the real iOS time, signal, and battery do not appear twice.
